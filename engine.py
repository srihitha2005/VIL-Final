...
class Engine():
        ...
        for batch_idx, (input, target) in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
            ...
            if self.distill_head is not None and task_id > 0:
                ...
                with torch.no_grad():
                    ...
                    teacher_feat = self.teacher_model.forward_features(input).detach() 
    ...
    def post_train_task(self, model: torch.nn.Module, data_loader_val: Iterable, task_id: int):
        ...
        # 5. Store FULL Teacher Model for Feature Distillation (LwF) ⬅️ PUT IT HERE
        if task_id < self.args.num_tasks - 1:
            ...
            # Ensure gradients are disabled for the teacher model copy
